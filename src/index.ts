...
async function main(): Promise<void> {
  ...
  const wa = new BaileysWhatsAppClient(connection, logger, config.operationTimeoutMs);
  ...
  const services: Services = {
    ...
    removal: new RemovalService(wa, logger, { batchSize: config.removeBatchSize, batchDelayMs: config.removeBatchDelayMs }),
  };
  ...
  const shutdown = async (reason: string, exitCode: number) => {
    ...
    if (services.removal.getRunning()) {
      logger.warn('Waiting for the running removal to stop after its current batch');
      services.removal.requestCancel();
      const idle = await services.removal.waitForIdle(SHUTDOWN_WAIT_MS);
      if (!idle) logger.error('Removal did not finish in time; exiting anyway (check the group for its final state)');
    }
